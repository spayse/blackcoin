...
bool AppInit2()
{
    ...
    // Blackcoin: Keep irc seeding on by default for now.
//    if (fTestNet)
//    {
...
//    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Blackcoin is probably already running."), GetDataDir().string().c_str()));
